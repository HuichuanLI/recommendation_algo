...
class AttentionPoolingLayer(Layer):
    ...
    def call(self, inputs):
        ...
        key_masks = tf.sequence_mask(self.user_behavior_length,
                                     keys.shape[1])  # (None, 1, max_len)  这里注意user_behavior_length是(None,1)
...
def interest_evolution(concat_behavior, query_input_item, user_behavior_length, neg_concat_behavior, gru_type="GRU",
                       use_neg=True):
    aux_loss = None
    use_aux_loss = None
    embedding_size = None
    ...
    rnn_outputs = DynamicGRU(embedding_size, return_sequence=True)(
        [concat_behavior, user_behavior_length])  # (None, max_len, embed_dim)
    ...
    if gru_type == "GRU":
        rnn_outputs2 = DynamicGRU(embedding_size, return_sequence=True)(
            [rnn_outputs, user_behavior_length])  # (None, max_len, embed_dim)
        ...
        if gru_type == "AIGRU":
            ...
            final_state2 = DynamicGRU(embedding_size, gru_type="GRU", return_sequence=False)(
            ...
            final_state2 = DynamicGRU(embedding_size, gru_type=gru_type, return_sequence=False)(
                [rnn_outputs, user_behavior_length, Permute([2, 1])(scores)])
        hist = final_state2
...
def DIEN(feature_columns, behavior_feature_list, behavior_seq_feature_list, neg_seq_feature_list, use_neg_sample=False,
         alpha=1.0):
    # 构建输入层
    ...
    # 兴趣进化层的计算过程
    dnn_seq_input, aux_loss = interest_evolution(keys_emb, query_emb, user_behavior_length, neg_concat_behavior,
                                                 gru_type="AUGRU")
...
if __name__ == "__main__":
    ...
    feature_columns += [
        VarLenSparseFeat('hist_movie_id', vocabulary_size=max(samples_data["movie_id"]) + 1, embedding_dim=8,
                         maxlen=50)]
    feature_columns += [
        VarLenSparseFeat('neg_hist_movie_id', vocabulary_size=max(samples_data["movie_id"]) + 1, embedding_dim=8,
                         maxlen=50)]
    ...
    """构建DIN模型"""
    history = DIEN(feature_columns, behavior_feature_list, behavior_seq_feature_list, neg_seq_feature_list,
                   use_neg_sample=True)
