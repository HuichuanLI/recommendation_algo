...
def ESSM(dnn_feature_columns, num_experts=3, expert_dnn_hidden_units=(256, 128), tower_dnn_hidden_units=(64,),
    ...
    ctr_pred = tf.math.sigmoid(ctr_logit)
    cvr_pred = tf.math.sigmoid(cvr_logit)

    ctcvr_pred = tf.keras.layers.Multiply(name=task_names[1])([ctr_pred, cvr_pred])  # CTCVR = CTR * CVR

    model = tf.keras.models.Model(inputs=input_layers, outputs=[ctr_pred, ctcvr_pred])
...
if __name__ == "__main__":
    ...
    model = ESSM(fixlen_feature_columns, task_types=['binary', 'binary'],
